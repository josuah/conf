...
class DBText(object):
  ...
  def _RunSelect(self):
        ...
        for col in self.columns:
          if col in self.aliases:
            ...
            for concat_col in self.aliases[col]:
              ...
              elif concat_col not in list(self.header.keys()):
            ...
            my_match = col.strip("'")
          elif col not in list(self.header.keys()):
            raise ExecuteError('Table %s does not have a column %s' %
                               (self.table, col))
          else:
            my_match = row[col]
    ...
    if self.order_by:
      if self.order_by not in list(self.header.keys()):
  ...
  def _MatchRow(self, row):
    ...
    for condition in self.conditions:
      ...
      if condition not in list(self.header.keys()):
        match = False
        break
      else:
  ...
  def _PositionByCol(self, col):
    ...
    Raises:
      ExecuteError: invalid column
    """
    if col not in list(self.header.keys()):
  ...
  def OpenTable(self):
    ...
    for cond in self.conditions:
      if cond not in list(self.header.keys()):
    ...
    for target in self.targets:
      if target not in list(self.header.keys()):
